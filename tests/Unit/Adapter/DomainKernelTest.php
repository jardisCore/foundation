...
class DomainKernelTest extends TestCase
{
    ...
    public function testSharedConfigRootIsNullByDefault(): void
    {
        $kernel = new DomainKernel($this->appRoot, $this->appRoot);

        $this->assertNull($kernel->getSharedConfigRoot());
    }

    public function testSharedConfigRootIsStoredWhenProvided(): void
    {
        $kernel = new DomainKernel(
            ...
            null,
            null,
            '/path/to/SharedConfig'
        );

        $this->assertSame('/path/to/SharedConfig', $kernel->getSharedConfigRoot());
    }

    public function testSharedConfigOverridesFoundation(): void
    {
        // Create temp SharedConfig directory with .env
    ...
    public function testDomainOverridesSharedConfig(): void
    {
        // Create temp SharedConfig directory
        ...
        // Domain should override SharedConfig (BC is King)
    ...
    public function testMissingSharedConfigDirIsGracefullySkipped(): void
    {
        $kernel = new DomainKernel(
            $this->appRoot,
            $this->appRoot,
            null,
            null,
            '/nonexistent/SharedConfig'
        );

        // Should not throw, env loading works fine without SharedConfig
        $env = $kernel->getEnv();
        $this->assertIsArray($env);
    }
}
