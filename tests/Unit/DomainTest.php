...
class DomainTest extends TestCase
{
    ...
    public function testGetClassVersionReturnsNullByDefault(): void
    {
        $domain = new class extends Domain {
            public function exposeClassVersion()
            {
    ...
    public function testCreateKernelIsOverridable(): void
    {
        $domain = new class extends Domain {
            ...
            protected function createKernel(
                ...
                return new class ($appRoot, $domainRoot, $classVersion, $resources, $sharedConfigRoot) extends DomainKernel {
    ...
    public function testGetSharedConfigRootReturnsNullWhenNoSharedConfigDir(): void
    {
        $domain = new class extends Domain {
            public function __construct()
            {
                // Point to a directory without SharedConfig sibling
                $this->domainRoot = sys_get_temp_dir();
            }

            public function exposeSharedConfigRoot(): ?string
            {
                return $this->getSharedConfigRoot();
            }
        };

        $this->assertNull($domain->exposeSharedConfigRoot());
    }

    public function testGetSharedConfigRootReturnsPathWhenSharedConfigDirExists(): void
    {
        // Create temp structure: parent/SharedConfig/ + parent/Domain/
        $parent = sys_get_temp_dir() . '/jardis-domain-test-' . uniqid();
        $domainDir = $parent . '/TestDomain';
        $sharedDir = $parent . '/SharedConfig';
        ...
        $this->assertSame($sharedDir, $domain->exposeSharedConfigRoot());
