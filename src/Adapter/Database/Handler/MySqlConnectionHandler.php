 ...
 *
 * Responsibility: Create MySQL connection configuration from External PDO or ENV.
 *
 * Supports two modes:
 ...
 * Optional ENV variables: DB_{PREFIX}_PASSWORD, DB_{PREFIX}_PORT, DB_{PREFIX}_CHARSET, DB_{PREFIX}_PERSISTENT
 */
class MySqlConnectionHandler
{
     ...
     * @return array{
     *     config: MySqlConfig|ExternalConfig,
     ...
     *
     * @param \JardisPort\Foundation\ResourceRegistryInterface $resources
     ...
     *
     * @return array{config: MySqlConfig, driverClass: class-string<DbConnectionInterface>, persistent: bool}
    ...
    private function createFromEnv(DomainKernelInterface $kernel, string $prefix): array
    {
        ...
        $mysqlConfig = new MySqlConfig(
            ...
            database: $database,
            port: (int) ($kernel->getEnv("DB_{$prefix}_PORT") ?? 3306),
            charset: $kernel->getEnv("DB_{$prefix}_CHARSET") ?? 'utf8mb4'
        );
        ...
        foreach ($options as $key => $value) {
            if ($value === null || (is_string($value) && trim($value) === '')) {
                throw new InvalidArgumentException(
                    "MySQL connection requires 'DB_{$prefix}_{$key}' environment variable. " .
                    "Example: DB_{$prefix}_HOST=localhost, DB_{$prefix}_USER=root, DB_{$prefix}_DATABASE=app_db"
                );
            }
        }
    }
