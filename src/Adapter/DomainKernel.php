...
class DomainKernel implements DomainKernelInterface
{
    ...
    private ?string $sharedConfigRoot;
    ...
    public function __construct(
        ...
        $this->sharedConfigRoot = $sharedConfigRoot;
    }

    public function getSharedConfigRoot(): ?string
    {
        return $this->sharedConfigRoot;
    }
     ...
     *
     * Load order: AppRoot(public) → Foundation(private) → SharedConfig(private) → Domain(private)
    ...
    private function loadEnvironment(): void
    {
        ...
        // 3. Load SharedConfig (private, organization-wide infrastructure config)
        if (
            $this->sharedConfigRoot !== null
            && is_dir($this->sharedConfigRoot)
            && file_exists($this->sharedConfigRoot . '/.env')
        ) {
            $sharedEnv = $dotEnv->loadPrivate($this->sharedConfigRoot);
