...
- **SharedConfig** — Organization-wide infrastructure configuration as sibling directory, auto-detected
- **Cross-Domain Connection Sharing** — Multiple kernels automatically share connections
- **Dynamic Class Versioning** — Load different implementations by version at runtime
...
class EcommerceDomain extends Domain
{
    protected function createKernel(
        ...
        return new EcommerceKernel($appRoot, $domainRoot, $classVersion, $resources, $sharedConfigRoot);
    }
}
```
...
Organization-wide infrastructure settings can be shared across domains via a `SharedConfig/` directory placed as a sibling of the domain root:

```
project/
├── SharedConfig/       # Shared .env for all domains
│   └── .env
├── EcommerceDomain/
...
```
AppRoot (public) → Foundation (private) → SharedConfig (private) → Domain (private)
```

`Domain` auto-detects `SharedConfig/` as sibling of `domainRoot`. No configuration needed.
